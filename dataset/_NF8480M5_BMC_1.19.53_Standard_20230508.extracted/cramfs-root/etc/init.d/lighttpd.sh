...
#add by gzz def文件是初始conf文件，high是根据高安全要求改变安全插件的配置文件，conf是应用配置文件的路径 20220825
def_lighttpd_file="/etc/defconfig/lighttpd1.1.conf"
high_lighttpd_file="/etc/defconfig/lighttpd_security_high.conf"
conf_lighttpd_file="/conf/lighttpd1.1.conf"

#add by gzz  enable_TLS_1_2_file是一个标识符，判断是否启用高安全需求 20220825
...
then 
	...
	if [ "$VAL" == "" ]; then
		...
		then
			...
			check_bkupconf
			if [ $? == 1 ]; then
				#If there is bkupconf in fstab, copy lighttpd1.1.conf to /bkupconf/
				echo "Copying /etc/defconfig/lighttpd.conf to bkupconf Lighttpd configurations "
				cp $conf_lighttpd_file /bkupconf/lighttpd1.1.conf
			fi
		else
			#和默认文件一样，未启动高安全需求（和高安全文件一样）
			echo "lighttpd1.1.conf is same to default lighttpd.conf!"
		fi
	else
		if  test -f "$enable_TLS_1_2_file"
		then
			#和默认文件不一样，启动了高安全需求（和高安全文件一样）
			echo "lighttpd1.1.conf is same to high lighttpd.conf!"
		else
	...
	if test -f "$enable_TLS_1_2_file" #判断是否启用高安全模式
		...
		echo "Copying lighttpd1.1.conf to conf "
		cp $def_lighttpd_file $conf_lighttpd_file
	fi
	check_bkupconf
	if [ $? == 1 ]; then
...
if [ -x /usr/local/sbin/lighttpd ]; then
    if ! `test -e /var/run/lighttpd.pid`; then
        /usr/local/sbin/lighttpd -f /conf/lighttpd1.1.conf -m /usr/local/lib
