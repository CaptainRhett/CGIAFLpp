...
# 4 - The domain name for TSIG authentication
# 5 - TSIG Authentication Enable
# 6 - TTL value (optional argument)
# 7 - Interface
...
NSLOOKUP_BINARY=/usr/bin/nslookup
if [ -r "/conf/tsig.private" ];then
TSIG_PRIVATE="/conf/tsig.private"
else
 TSIG_PRIVATE="/conf/tsig_"$7".private"
fi
...
#      	echo "update delete $2 A" > $NSUPDATE_TEMP_FILE
        ...
        if [ $IPv6 ]; then
            ...
            echo "update delete $REV_IP6""ip6.arpa PTR $2" >> $NSUPDATE_TEMP_FILE
            echo "send" >> $NSUPDATE_TEMP_FILE
...
#     	echo "reversed ip is " $reversedip
        #echo "Reversed IPv4 is " $reversedip
      	echo "update delete $reversedip.in-addr.arpa PTR $2" >> $NSUPDATE_TEMP_FILE
      	echo "send" >> $NSUPDATE_TEMP_FILE
        fi
        ...
        if [ $5 -eq 0 ]; then
            ...
            SECRET=`grep "Key" $TSIG_PRIVATE | awk -F": " '{print $2}'`
            $NSUPDATE_BINARY -v -y $4:$SECRET $NSUPDATE_TEMP_FILE
        fi
        ;;
      register)
      	if [ -z "$2" ] || [ -z "$3" ]; then
...
#    		echo -n "Using default TTL of one day"
	   	...
	   	if [ -e "$NSLOOKUP_BINARY" ]; then 		
	   		OLDDOMAIN=`nslookup $3 | tail -1 | head -2 |  awk '{print $4}'`
	   	else
	   		OLDDOMAIN=$2
	   	fi
#		echo "update delete $2 A" > $NSUPDATE_TEMP_FILE
        ...
        if [ $OLDDOMAIN ]; then
            echo "update delete $OLDDOMAIN $RECORD_TYPE $3" > $NSUPDATE_TEMP_FILE
            echo "send" >> $NSUPDATE_TEMP_FILE
        ...
        if [ $IPv6 ]; then
            ...
            #echo "Reversed IPv6 is " $REV_IP6
            if [ $OLDDOMAIN ]; then
            	echo "update delete $REV_IP6""ip6.arpa PTR $OLDDOMAIN" >> $NSUPDATE_TEMP_FILE
            	echo "send" >> $NSUPDATE_TEMP_FILE
            fi
...
#		echo "reversed ip is " $reversedip
       	if [ $OLDDOMAIN ]; then
       		echo "update delete $reversedip.in-addr.arpa PTR $OLDDOMAIN" >> $NSUPDATE_TEMP_FILE
       		echo "send" >> $NSUPDATE_TEMP_FILE
       	fi
