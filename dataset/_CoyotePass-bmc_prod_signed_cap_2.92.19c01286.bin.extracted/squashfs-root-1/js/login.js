...
/* If user already login and open a new web page, use this to redirect
   it to the main page before cgi check. It will check there if the
   session is invalid. */
var loginFast = 1;
var cookieAttmptCnt =  ReadSessionStorage("LoginFast");
if(cookieAttmptCnt)
    loginFast = parseInt(cookieAttmptCnt);

if(ReadSessionStorage("Authenticated") && loginFast) {
    // fast login mode: when already authenticated.
    CreateSessionStorage("LoginFast", 0); // if fast login failed next time use slow login
    top.location.href = "/cgi/url_redirect.cgi?url_name=mainmenu";
}
else {
    // slow login mode: check cgi result.
...
function PageInit()
{
    ...
    var available_sessions = ReadSessionStorage("AVAILABLE_SESSION");
    RemoveSessionStorage("AVAILABLE_SESSION");
...
function language_Change()
{
    ...
    if(document.getElementById("language_choice").value == "en")
    {
        ...
        CreateSessionStorage("language","Chinese");
