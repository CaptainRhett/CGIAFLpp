...
function PageInit()
{
    ...
    var bond_tb = document.getElementById("bond_tb");
    bond_tb.style.display = "none";
...
function OutputString() {
    ...
    document.getElementById("failover_lbl").textContent = lang.LANG_CONF_NETWORK_IPV6_FAILOVER;
...
function enableIPEditor(enable) {
    ...
    var item = document.getElementById("_prefix");
    item.disabled = disabled;
...
{
    "use strict";
    var ajax_url = '../cgi/netip6cfg.cgi';
    var ajax_param = 'ipv6conf.xml';
    var ajax_data = geneConfigXML(channel, nicnum, "0", "0", "0", "0", "0","::", "00-00-00-00-00-00", "64", "::", "0", "::", "::");
...
function updateConfig(response) {
    ...
    if (response.readyState == 4 && response.status == 200) {
        ...
        if(xml_obj != null) {
           ...
           setTimeout(function(){ location.href = "../cgi/url_redirect.cgi?url_name=config_ipv6"; }, 2000);
        }
    }
}
...
function updateChannelList(root) {
    ...
    if (initFailOver == true) {
        if (item.checked == true) {
            ...
            for(var i = 0; i < totalNIC; i++) {
                channel_id = node[i].getAttribute("AVAILABLE");
    ...
    if(selector != null) {
        ...
        for(var idx = 0; idx < numChannels; idx++) {
            channel_id = node[idx].getAttribute("AVAILABLE");
...
function updateNICConfiguration(root) {
    ...
    if(node != null) {
       enable_failover = node.getAttribute("FAILOVER_EN");
       failover_slave_num = node.getAttribute("FAILOVER_SLAVE_NUM");
       item = document.getElementById("_enableLanFailover");
    ...
    if(node != null) {
       ...
       var item = document.getElementById("_ipAddress");
       item.value = attr;
       var attr = node.getAttribute("BMC_PREFIX");
       var item = document.getElementById("_prefix");
       item.value = attr;
...
function saveConfiguration() {
    ...
    UtilsConfirm(lang.LANG_CONF_NETWORK_WARNING2, {
                 ...
                 if (isNaN(prefix) || prefix < 0 || prefix > 128) {
                     alert(lang.LANG_CONFIG_NETWORK_ERR_INVALID_PREFIX);
                     return;
                 }
